...
if [ ! -f ./.installed ] | [ ! -d ./miniconda ]; then
    ...
    ./miniconda/envs/hopsworks/bin/pip install jupyterlab==2.3.2 jupyter scikit-learn==1.0.2 matplotlib==3.5.2 seaborn==0.11.2 streamlit==1.11.0 plotly==5.9.0 joblib==1.1.0 faker xgboost folium streamlit_folium 

    # Installing hopsworks should be the last pip install
    ./miniconda/envs/hopsworks/bin/pip install hopsworks==3.0.3 --no-cache-dir
    touch ./.installed
fi
...
if [ -f ./miniconda/envs/hopsworks/bin/jupyter ]; then
    ./miniconda/envs/hopsworks/bin/jupyter trust src/03-module/1_backfill_cc_feature_groups.ipynb
